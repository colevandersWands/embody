...
2. **Defaults** (`default-config.ts`) - Complete default configuration values
...
- `default-config.ts` exports `defaultConfig`
...
// Expands to full lang structure from default-config.ts
{
  lang: {
    semantics: true,
    bindings: { /* ... */ },
...
2. **Set Defaults** in `default-config.ts`:
...
configuring/
  ...
  default-config.ts     # Complete default configuration
  presets/
